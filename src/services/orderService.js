...
const orderService = {
  ...
  confirmAndDeliver(orderId) {
    ...
    const deliveredData = JSON.stringify(accounts);

    // Update order status and delivered data
    db.prepare(`
      UPDATE orders SET status = 'delivered', paid_at = CURRENT_TIMESTAMP, delivered_at = CURRENT_TIMESTAMP, delivered_data = ?
      WHERE id = ?
    `).run(deliveredData, orderId);

    return { success: true, accounts, order: { ...order, status: 'delivered', delivered_data: deliveredData } };
  },
   ...
   */
  manualDeliver(orderId, accounts) {
    const deliveredData = accounts ? JSON.stringify(accounts) : null;
    db.prepare(`
      UPDATE orders SET status = 'delivered', delivered_at = CURRENT_TIMESTAMP, delivered_data = ?
      WHERE id = ?
    `).run(deliveredData, orderId);
  },
